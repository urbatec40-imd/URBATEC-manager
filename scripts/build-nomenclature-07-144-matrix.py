...
def decode_row(line: str):
    s = clean(line); m = REGIME_RE.search(s)
    if not m: return None
    left, right = clean(s[:m.start()]), clean(s[m.end():])
    if not left: return None
    nums = re.findall(NUMBER, right); rayon = nums[0] if nums else ""
    compact = re.findall(r"[x×]|[^\s]+", right[right.find(nums[0]) + len(nums[0]):] if nums else right)
    last4 = compact[-4:] if len(compact) >= 4 else []
    docs = {"impact": False, "danger": False, "notice": False, "rapportDangereux": False}
    if last4:
        docs = {"impact": last4[0].lower() in {"x", "×"}, "danger": last4[1].lower() in {"x", "×"}, "notice": last4[2].lower() in {"x", "×"}, "rapportDangereux": last4[3].lower() in {"x", "×"}}
...
def looks_like_non_designation(text: str) -> bool:
    t = clean(text).lower()
    starters = ("voir ", "visé", "vises ", "à l’exclusion", "a l'exclusion", "la quantité", "le nombre", "la capacité", "la puissance", "la surface", "le volume", "1.", "2.", "3.", "4.", "cas ")
    return t.startswith(starters) or t.startswith("condition") or re.match(r"^\d+[.)]\s+", t) is not None

def download_pdf() -> None:
    TMP.mkdir(parents=True, exist_ok=True)
    if PDF.exists() and PDF.stat().st_size > 100_000: return
    last_error = None
    for url in PDF_URLS:
        try:
            urllib.request.urlretrieve(url, PDF)
            if PDF.stat().st_size > 100_000: return
        except Exception as exc: last_error = exc
    raise SystemExit(f"Impossible de télécharger le PDF officiel : {last_error}")

def extract() -> list[dict]:
    rows, current_code, current_group, page_number = [], None, "", 0
    with pdfplumber.open(PDF) as pdf:
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            for raw_line in text.splitlines():
                line = clean(raw_line)
                if not line or "JOURNAL OFFICIEL" in line or line.startswith("ANNEXE"): continue
                m = RUBRIQUE_RE.match(line)
                if m:
                    current_code = m.group(1); current_group = ""; continue
                if current_code is None: continue
                ...
                if decoded:
                    ...
                    else: lo = hi = None; lo_inc, hi_inc, unit = True, False, infer_unit(condition_text)
                    rows.append({"rubrique": current_code, "criterion": current_group or "", "group": current_group, "rawCondition": condition_text, "min": lo, "minInclusive": lo_inc, "max": hi, "maxInclusive": hi_inc, "unit": unit, "regime": regime, "rayon": rayon, "documents": docs, "sourcePage": page_number})
                    continue
                if looks_like_group(line): current_group = line
    return rows
        ...
        for page_number, page in enumerate(pdf.pages, start=1):
            ...
            for raw in text.splitlines():
                ...
                if m:
                    ...
                    # The text appearing on the same line as the rubrique number is the primary designation.
                    if rest: designations[current] = rest
                    continue
                if current is None or current in designations: continue
                if not line or "JOURNAL OFFICIEL" in line or line.startswith("ANNEXE"): continue
...
def build() -> None:
    download_pdf()
    rows = extract(); designations = extract_designations(); grouped: dict[str, dict] = {}
    for row in rows:
        code = row["rubrique"]
        item = grouped.setdefault(code, {"rubrique": code, "famille": code[:2] + "00", "familleLabel": FAMILIES.get(code[:2] + "00", "Installation classée"), "designation": designations.get(code, ""), "decisionRows": [], "source": "Décret exécutif n° 07-144 du 19 mai 2007", "sourceUrl": PDF_URLS[0]})
        item["decisionRows"].append(row)
    for code, item in grouped.items():
        item["designation"] = clean(designations.get(code) or item["designation"] or item["decisionRows"][0].get("criterion", ""))[:500]
        unique, seen = [], set()
    ...
    data = {"version":"07-144","date":"19 mai 2007","sourceUrl":PDF_URLS[0],"families":[{"code":k,"label":v} for k,v in FAMILIES.items()],"rubriques":rubriques,"generated":True,"generatorVersion":"matrix-v2","generatorNote":"Designation = primary title line; decisionRows = matrix decision lines. Non-designation notes are not promoted to activity titles."}
