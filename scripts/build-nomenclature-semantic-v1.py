...
def regime_of(s: str)->str|None:
    m=REGIME_RE.fullmatch(clean(s)); return ("APAPC" if m and m.group(1).upper()=="PAPC" else m.group(1).upper()) if m else None
...
def group_lines(page):
    ...
    for w in sorted(words,key=lambda z:(float(z["top"]),float(z["x0"]))):
        if not buckets or abs(float(w["top"])-float(buckets[-1][0]["top"]))>3.5: buckets.append([w])
        else: buckets[-1].append(w)
    ...
    for line in lines[:35]:
        for w in line:
            ...
            for key, labels in DOC_HEADERS.items():
                if any(label in t for label in labels): pos[key]=float(w["x0"])
            if "d’affichage" in t or "d'affichage" in t: pos["rayon"]=float(w["x0"])
            if "type" in t and "autorisation" in t: pos["regime"]=float(w["x0"])
    return pos
...
def decision_from_line(line, headers):
    regime_idx=None; regime=None
    for i,w in enumerate(line):
        r=regime_of(w["text"])
        if r: regime_idx=i; regime=r; break
    if regime is None: return None
    docs={k:False for k in DOC_HEADERS}; rayon=""
    for w in line[regime_idx+1:]:
        t=clean(w["text"])
        x=float(w["x0"])
        if t.lower().strip(".") in XVAL:
            k=classify_x(x,headers)
            if k in docs: docs[k]=True
        elif not rayon and re.search(r"\d",t):
            if headers.get("rayon",10**9) <= x < min([v for k,v in headers.items() if k in DOC_HEADERS] or [10**9]): rayon=t
...
def parse():
    ...
    if not PDF.exists() or PDF.stat().st_size<100_000: urllib.request.urlretrieve(URL,PDF)
    by_code={}
    with pdfplumber.open(PDF) as pdf:
        for pno,page in enumerate(pdf.pages,1):
            ...
            current=None; pre=[]; pending=None
            for line in lines:
                text=clean(" ".join(w["text"] for w in line))
                code=code_of(text)
                if code:
                    if current and pending:
                        pending["condition"]=clean(pending["condition"]+" "+" ".join(pre)); current["decisionRows"].append(pending); pending=None; pre=[]
                    current=code
                    by_code.setdefault(code,{"rubrique":code,"famille":code[:2]+"00","familleLabel":FAMILIES.get(code[:2]+"00",""),"designation":"","decisionRows":[]})
                    after=re.sub(r"^.*?\b"+re.escape(code)+r"\b","",text,1).strip(" :-")
                    if after and not is_family(code): pre=[after]
                    else: pre=[]
                    continue
                if not current or is_family(current): continue
                d=decision_from_line(line,headers)
                if d:
                    if pending:
                        pending["condition"]=clean(pending["condition"]+" "+" ".join(pre)); current_item=by_code[current]; current_item["decisionRows"].append(pending)
                    pending=d; pre=[]
                else:
                    if text and not text.startswith("Nota") and not text.startswith("ANNEXE"):
                        pre.append(text)
            if current and pending:
                pending["condition"]=clean(pending["condition"]+" "+" ".join(pre)); by_code[current]["decisionRows"].append(pending)
    rubriques=[]
