...
DEFAULT_COLUMNS = {"regime": 250.0, "rayon": 310.0, "impact": 355.0, "danger": 405.0, "notice": 444.0, "rapportDangereux": 486.0}
...
def header_positions(lines):
    pos = dict(DEFAULT_COLUMNS)
    for line in lines[:90]:
        for w in line:
            t = clean(w["text"]).lower()
            x = float(w["x0"])
            if "autorisation" in t and x < 300:
                pos["regime"] = x
            if "d’affichage" in t or "d'affichage" in t:
                pos["rayon"] = x
            if t in {"impact", "d’impact", "d'impact"}:
                if x < 400:
                    pos["impact"] = x
                else:
                    pos["notice"] = x
            if t == "danger":
                pos["danger"] = x
            if "produits" in t:
                pos["rapportDangereux"] = x
    return pos


def classify_x(x: float, headers):
    candidates = [(abs(x - p), key) for key, p in headers.items() if key in DOC_HEADERS]
...
def decision_from_line(line, headers):
    ...
    rayon = ""
    doc_x = min([v for k, v in headers.items() if k in DOC_HEADERS] or [float("inf")])
    for w in line[regime_idx + 1:]:
        ...
        if t.lower().strip(".") in XVAL:
            ...
            if k in docs:
                docs[k] = True
        elif not rayon and re.fullmatch(r"\d+(?:[.,]\d+)?", t) and x < doc_x:
            rayon = t
...
def parse():
        ...
        for pno, page in enumerate(pdf.pages, 1):
            ...
            for line in lines:
                ...
                if code:
                    ...
                    if not is_family(current_code) and after:
                        # Split the inline Rubrique title from an inline decision.
                        tokens = line
                        decision = decision_from_line(tokens, headers)
                        if decision:
                            pending = decision
                            pre = []
                            title_tokens = []
                            for w in line:
                                if regime_of(w["text"]):
                                    break
                                if clean(w["text"]) != code:
                                    title_tokens.append(clean(w["text"]))
                            item["designation"] = clean(" ".join(title_tokens))
                        else:
                            item["designation"] = after
                            pre = [after]
                    else:
                        pre = []
                    continue
                ...
                if decision:
                    ...
                    pending = decision
                    pre = []
                elif text and not text.startswith("Nota") and not text.startswith("ANNEXE"):
                    pre.append(text)
    ...
    for code, item in by_code.items():
        ...
        if not item["designation"]:
            item["designation"] = code if not rows else rows[0].get("condition", code)
    ...
    for code in ["1240", "1310", "2110", "2120", "2122"]:
        ...
        print(f"TEST {code}: {json.dumps(item, ensure_ascii=False)[:2200] if item else 'NOT FOUND'}")
