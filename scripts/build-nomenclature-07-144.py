...
def parse() -> dict:
    ...
    raw = clean_text(TXT.read_text(encoding="utf-8", errors="replace"))
    ...
    for line in lines:
        ...
        if page_match and "JOURNAL OFFICIEL" in line:
    ...
    seen = set()
    for r in rubriques:
        if not r.get("rawDescription"):
            continue
        key = (r["rubrique"], r["rawDescription"][:160])
        if key in seen:
            continue
        seen.add(key)
