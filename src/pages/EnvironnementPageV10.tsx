...
export function EnvironnementPageV10({ clientName, dossierNumero, onBack }: { clientName: string; dossierNumero?: string; onBack: () => void }) {
  ...
  return <div className="space-y-6">
    ...
    {!selected && <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      ...
      {direct && <div className="mt-4 border border-emerald-200 bg-emerald-50 rounded-lg p-4 flex items-center gap-3"><div className="flex-1"><div className="text-sm font-semibold">Rubrique {direct.rubrique}</div><div className="text-xs text-gray-600 mt-1">{direct.designation}</div></div><button type="button" onClick={() => accept(direct)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold"><Check size={14}/> Accepter</button></div>}
      {!direct && query.trim().length >= 2 && <div className="mt-4"><div className="text-xs font-semibold text-gray-500 mb-2">Rubriques / activités suggérées — par ordre de correspondance</div>{suggestions.length ? <div className="border rounded-lg divide-y max-h-96 overflow-y-auto">{suggestions.map((c: ActivityCandidate, i) => { const r = dataset.rubriques.find(x => x.rubrique === c.rubrique); return <div key={`${c.rubrique}-${c.designation}`} className="p-4 flex items-center gap-3"><div className="w-10 text-xs font-bold text-gray-400">#{i + 1}</div><div className="flex-1"><div className="font-medium text-sm">{c.designation}</div><div className="text-xs text-gray-500 mt-1">{c.familleLabel} · Rubrique {c.rubrique} · Correspondance {Math.min(99, Math.max(1, Math.round(c.score)))}%</div></div><button type="button" disabled={!r} onClick={() => r && accept(r)} className="inline-flex items-center gap-1 px-3 py-1.5 rounded-lg bg-emerald-600 text-white text-xs font-semibold disabled:opacity-40"><Check size={14}/> Accepter</button></div>; })}</div> : <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 text-sm text-amber-800">Aucune proposition suffisamment fiable.</div>}</div>}
    </div>}

    {selected && <>
      <div className="bg-white rounded-xl border border-emerald-200 shadow-sm p-5"><div className="text-sm font-semibold text-emerald-800">Rubrique acceptée</div><div className="text-xl font-bold mt-1">{selected.rubrique}</div><div className="text-sm text-gray-700 mt-1">{selected.designation}</div><div className="text-xs text-gray-500 mt-1">{selected.familleLabel}</div><button type="button" onClick={reset} className="mt-3 text-xs text-sky-700 hover:underline">Changer</button></div>
