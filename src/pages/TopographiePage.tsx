...
export function TopographiePage() {
  ...
  const searchParcel = () => { if (surface) setSearched(true); };
  ...
  return (
    <div className="space-y-5">
      <div><h1 className="text-2xl font-bold text-slate-800">Topographie / Cadastre</h1><p className="text-sm text-slate-500 mt-1">Recherche parcellaire et localisation des îlots</p></div>
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <label className="text-sm font-medium text-slate-700">Commune<select value={commune} onChange={e => { setCommune(e.target.value); setSearched(false); }} className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5 bg-white"><option value="">Sélectionner la commune</option>{COMMUNES.map(c => <option key={c}>{c}</option>)}</select></label>
          <label className="text-sm font-medium text-slate-700">Section<input value={section} onChange={e => { setSection(e.target.value); setSearched(false); }} placeholder="Ex. A" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5" /></label>
          <label className="text-sm font-medium text-slate-700">Îlot<input value={ilot} onChange={e => { setIlot(e.target.value); setSearched(false); }} placeholder="Ex. 125" className="mt-1 w-full rounded-lg border border-gray-300 px-3 py-2.5" /></label>
        </div>
        <div className="mt-4 flex flex-wrap items-center gap-3">
          <button disabled={!surface} onClick={searchParcel} className="inline-flex items-center gap-2 rounded-lg bg-slate-800 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40"><Search size={16} /> Rechercher / Zoom</button>
          <button disabled={!surface} onClick={addParcel} className="inline-flex items-center gap-2 rounded-lg bg-sky-600 px-4 py-2.5 text-sm font-semibold text-white disabled:opacity-40"><Plus size={16} /> Ajouter à la carte</button>
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm"><div className="h-[520px] relative bg-slate-100 overflow-hidden">
