...
function Info({ feature, onClose }: { feature: Parcel; onClose: () => void }) {
  ...
  return <div className="absolute z-[1000] bottom-4 left-4 max-w-sm rounded-xl bg-white shadow-xl border p-4">
    <div className="flex items-center justify-between gap-4"><strong>Parcelle cadastrale</strong><button type="button" onClick={onClose} className="text-slate-500"><X size={17}/></button></div>
    <div className="mt-3 grid grid-cols-3 gap-2 text-sm"><div className="rounded-lg bg-slate-50 p-2"><b className="block text-xs text-slate-500">Section</b>{sectionOf(p)||'—'}</div><div className="rounded-lg bg-slate-50 p-2"><b className="block text-xs text-slate-500">Ilot</b>{ilotOf(p)||'—'}</div><div className="rounded-lg bg-slate-50 p-2"><b className="block text-xs text-slate-500">Area</b>{areaOf(p)||'—'}</div></div>
  </div>;
}
...
export function TopographiePage() {
  ...
  const filtered = useMemo(() => { const q=commune.trim().toUpperCase(); if(!q)return COMMUNES; return COMMUNES.filter(c=>c.toUpperCase().includes(q)).sort((a,b)=>{const an=a.toUpperCase(),bn=b.toUpperCase();const as=an.startsWith(q),bs=bn.startsWith(q);if(as!==bs)return as?-1:1;return an.localeCompare(bn);}); },[commune]);
  ...
  const parcelResults = useMemo(() => {
    const s=digits(section), i=digits(ilot); if(!s && !i)return [];
    return features.map(feature=>({feature,s:sectionOf(feature.properties||{}),i:ilotOf(feature.properties||{})})).filter(x=>{
      const sv=digits(x.s),iv=digits(x.i); return (!s||sv===s)&&(!i||iv===i);
    }).sort((a,b)=>{const target=num(i); if(i){const d=num(a.i)-target,e=num(b.i)-target;if(Math.abs(d)!==Math.abs(e))return Math.abs(d)-Math.abs(e);} return num(a.s)-num(b.s);}).slice(0,100);
  },[features,section,ilot]);
  ...
  return <div className="space-y-5">
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[320px_1fr] gap-4"><aside className="bg-white border rounded-xl shadow-sm p-4"><h2 className="font-semibold text-slate-800 flex items-center gap-2"><MapPinned size={17}/> Recherche cadastrale</h2>
