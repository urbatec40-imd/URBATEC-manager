...
import { MapPinned, Plus, Trash2, Search, FolderOpen, Layers3 } from 'lucide-react';
...
function FitToFeature({ feature }: { feature?: Feature<Geometry> }) {
  ...
  if (feature) {
    ...
    if (layer) map.fitBounds(layer.getBounds(), { padding: [40, 40], maxZoom: 18 });
  }
  return null;
}
...
export function TopographiePage() {
  ...
  const [focus, setFocus] = useState<Feature<Geometry> | undefined>();
  const [status, setStatus] = useState('');
  const [loading, setLoading] = useState(false);

  const matching = useMemo(() => {
    if (!cadastre || !section.trim() || !ilot.trim()) return [];
    const sec = normalize(section); const ilo = normalize(ilot);
    return cadastre.features.filter(f => {
      const p = (f.properties ?? {}) as Record<string, unknown>;
      const s = normalize(prop(p, ['SECTION','SECTION_CAD','SEC','SECT']));
      const i = normalize(prop(p, ['ILOT','ILOTS','ILOT_CAD','NUMILOT','NUM_ILOT','PARCELLE','PARCEL']));
      return s === sec && i === ilo;
    });
  ...
  const loadShpFolder = async (files: FileList | null) => {
    ...
    setLoading(true); setStatus('Lecture du SHP...');
    try {
      ...
      const [shpBuffer, dbfBuffer, prjText] = await Promise.all([shpFile.arrayBuffer(), dbfFile.arrayBuffer(), prjFile?.text() ?? Promise.resolve(undefined)]);
      ...
      setSelected([]); setFocus(undefined);
      setStatus(`${combined.features.length.toLocaleString('fr-FR')} entités cadastrales chargées.`);
    } catch (e) {
      setCadastre(null); setStatus(`Erreur SHP : ${(e as Error).message}`);
    } finally { setLoading(false); }
  };

  const searchParcel = () => {
    if (!matching.length) { setStatus('Aucune parcelle trouvée pour cette Section / Îlot.'); return; }
    setFocus(matching[0]);
  ...
  const addParcel = () => {
    matching.forEach((feature, index) => {
      ...
      const area = numeric(prop(p, ['SURFACE','SUPERFICIE','AREA','AIRE','CONTENANCE']));
      const id = `${commune}-${s}-${i}-${index}`;
      setSelected(old => old.some(x => x.id === id) ? old : [...old, { id, commune, section: s, ilot: i, surface: area, feature }]);
    });
    ...
    <div className="space-y-5">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm hover:bg-slate-50">
          ...
          <input type="file" className="hidden" multiple webkitdirectory="true" onChange={e => loadShpFolder(e.target.files)} />
        </label>
      </div>
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-3">
          ...
          <div className="inline-flex items-center gap-2 rounded-lg border bg-slate-50 px-3 py-2 text-sm"><Layers3 size={16} /> Sources : OSM • Satellite • Topographique</div>
          ...
          <MapContainer center={[35.435, 7.143]} zoom={11} className="h-full w-full">
            <LayersControl position="topright">
              ...
              <LayersControl.BaseLayer name="Topographique">
                <TileLayer attribution="© OpenTopoMap contributors" url="https://{s}.tile.opentopomap.org/{z}/{x}/{y}.png" />
              </LayersControl.BaseLayer>
              {cadastre && <LayersControl.Overlay checked name="Cadastre SHP">
                <GeoJSON data={cadastre as any} style={() => ({ color: '#2563eb', weight: 1, fillOpacity: 0.08 })} />
              </LayersControl.Overlay>}
              {selected.length > 0 && <LayersControl.Overlay checked name="Parcelles sélectionnées">
                <>{selected.map(p => <GeoJSON key={p.id} data={p.feature as any} style={() => ({ color: '#dc2626', weight: 3, fillOpacity: 0.18 })} />)}</>
              </LayersControl.Overlay>}
            </LayersControl>
      ...
      {!cadastre && <div className="text-xs text-slate-500 bg-slate-50 border rounded-lg p-3">Chargez le dossier contenant les fichiers .SHP, .SHX, .DBF et .PRJ. Le traitement des données cadastrales reste local; Internet est utilisé uniquement pour les fonds cartographiques.</div>}
    </div>
  );
}
