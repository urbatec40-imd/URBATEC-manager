...
import { ArrowLeft, Check, Calculator, FileText, SearchCheck, ShieldCheck, X } from 'lucide-react';
...
function fieldsFor(row: Row): ClassificationField[] {
  ...
  return getClassificationProfile(row.rubrique, conditionText(row))?.fields ?? [];
}

function classificationFor(row: Row, values: Record<string, string>): ClassificationResult | null {
  const profile = getClassificationProfile(row.rubrique, conditionText(row));
...
export function EnvironnementPageV6({ clientName, dossierNumero, onBack }: { clientName: string; dossierNumero?: string; initialPrestation?: string; onBack: () => void }) {
  ...
  return <div className="space-y-6">
    ...
    {selected && <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5"><div className="flex items-center gap-2 mb-4"><Calculator size={18} className="text-emerald-600"/><h2 className="font-semibold text-gray-800">Données nécessaires au classement — Rubrique {selected.rubrique}</h2></div><div className="rounded-lg bg-slate-50 border border-slate-200 p-4 mb-4"><div className="font-semibold">{selected.designation}</div><div className="text-xs text-gray-500 mt-1">{selected.familleLabel}</div></div>
      {selectedFields.length === 0 ? <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800">La rubrique est acceptée. Les seuils détaillés de cette rubrique doivent encore être structurés dans le catalogue local avant calcul automatique.</div> : <><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">{selectedFields.map(f => <Field key={f.key} label={`${f.label}${f.unit ? ` (${f.unit})` : ''}`} required={f.required}><input className={inputCls} type={f.type === 'number' ? 'number' : 'text'} min={f.type === 'number' ? '0' : undefined} value={values[selected.id]?.[f.key] ?? ''} onChange={e => { setValues(v => ({ ...v, [selected.id]: { ...(v[selected.id] ?? {}), [f.key]: e.target.value } })); setResults(v => ({ ...v, [selected.id]: null })); setAcceptedClass(v => ({ ...v, [selected.id]: false })); }} /></Field>)}</div><button disabled={!hasEnoughForCalculation} onClick={calculateSelected} className="mt-5 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-sky-600 text-white text-sm font-semibold disabled:opacity-40"><Calculator size={16}/> Calculer le classement</button></>}

      {selectedResult && <div className="mt-5 border-t pt-5"><div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3"><Result label="Sous-rubrique" value={selectedResult.code}/><Result label="Régime" value={selectedResult.regime}/><Result label="Catégorie" value={selectedResult.categorie ?? '—'}/><Result label="Rayon" value={selectedResult.rayon ?? '—'}/></div><div className="mt-4 rounded-lg bg-emerald-50 border border-emerald-200 p-4"><p className="font-semibold text-emerald-900">{selectedResult.seuil}</p><p className="text-xs text-emerald-700 mt-1">Vérification finale à effectuer sur la rubrique réglementaire et ses éventuels textes modificatifs.</p></div><button onClick={acceptClassification} className="mt-4 inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-600 text-white text-sm font-semibold"><Check size={16}/> Accepter le classement</button></div>}
    </div>}

    {selected && acceptedClass[selected.id] && <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5"><div className="flex items-center gap-2 mb-4"><ShieldCheck size={18} className="text-emerald-600"/><h2 className="font-semibold text-gray-800">Exigences du dossier</h2></div>{selectedRequirements && <><div className="grid grid-cols-1 md:grid-cols-2 gap-3">{['Étude d’impact','Étude de dangers','Notice d’impact','Rapport sur les produits dangereux'].map(name => { const key = name === 'Étude d’impact' ? 'impact' : name === 'Étude de dangers' ? 'danger' : name === 'Notice d’impact' ? 'notice' : 'rapportDangereux'; const yes = selectedRequirements.docs.includes(name); return <div key={name} className={`rounded-lg border p-3 ${yes ? 'border-emerald-200 bg-emerald-50' : 'border-gray-200 bg-gray-50'}`}><div className="text-xs text-gray-500">{yes ? 'REQUIS' : 'Non identifié'}</div><div className="font-medium text-gray-800">{name}</div></div>})}</div><div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-3"><Result label="Régime" value={selectedResult?.regime ?? '—'}/><Result label="Rayon d’affichage" value={selectedRequirements.rayon ?? '—'}/></div><div className="mt-4 text-xs text-slate-600 bg-slate-50 border border-slate-200 rounded-lg p-3">Les exigences affichées sont celles identifiées dans la condition de classement disponible dans le catalogue local. Elles seront complétées lorsque les colonnes documentaires de toutes les rubriques auront été structurées.</div></>}</div>}

    <div className="bg-slate-50 rounded-xl border border-slate-200 p-5"><div className="flex items-center gap-2"><FileText size={17}/><h2 className="font-semibold">Documents environnementaux</h2></div><p className="text-xs text-gray-500 mt-2">Les documents apparaissent après acceptation du classement.</p></div>
