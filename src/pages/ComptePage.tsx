...
export function ComptePage() {
  ...
  if (!user) {
    return <div className="rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-800">Aucune session utilisateur active.</div>;
  }

  return (
    <div className="space-y-5">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="rounded-lg border bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase text-slate-500"><UserCircle2 size={15}/> Nom d'utilisateur</div><div className="mt-1 font-bold text-slate-800">{user.username || '—'}</div></div>
        <div className="rounded-lg border bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase text-slate-500"><UserCircle2 size={15}/> Nom complet</div><div className="mt-1 font-bold text-slate-800">{user.nom_complet || '—'}</div></div>
        <div className="rounded-lg border bg-slate-50 p-4"><div className="flex items-center gap-2 text-xs font-semibold uppercase text-slate-500"><ShieldCheck size={15}/> Rôle</div><div className="mt-1 font-bold text-slate-800">{user.role || '—'}</div></div>
      </div>
