...
function cleanText(v: string) { return v.replace(/Ã©/g,'é').replace(/Ã¨/g,'è').replace(/Ãª/g,'ê').replace(/Ã®/g,'î').replace(/Ã´/g,'ô').replace(/Ã¹/g,'ù').replace(/Ã§/g,'ç').replace(/Ã /g,'à').replace(/â€™/g,'’').replace(/dâ€™/g,'d’').replace(/lâ€™/g,'l’').replace(/\s+/g,' ').trim(); }
...
export function EnvironnementPageV5({ clientName, dossierNumero, onBack }: { clientName: string; dossierNumero?: string; initialPrestation?: string; onBack: () => void }) {
  ...
  function clearActivity() { setQuery(''); setComponents([]); setSelectedId(null); setValues({}); setAnalysed(false); }
  ...
  return <div className="space-y-6">
    <div className="flex items-center justify-between gap-3"><div className="flex items-center gap-3"><button onClick={onBack} className="p-2 rounded-lg border border-gray-300 hover:bg-gray-50"><ArrowLeft size={18} /></button><div><h1 className="text-2xl font-bold text-gray-800">Module Environnement</h1><p className="text-sm text-gray-500">{dossierNumero ?? 'Nouveau projet'}{clientName ? ` — ${clientName}` : ''}</p></div></div><span className="text-xs px-3 py-1.5 rounded-full bg-emerald-50 text-emerald-700 border border-emerald-200">Analyse multi-rubriques</span></div>
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-5">
      <Field label="Description du projet / activité" required><div className="relative"><textarea className={`${inputCls} min-h-[110px] pr-12`} value={query} onChange={e => { const v=e.target.value; setQuery(v); setAnalysed(false); if(!v.trim()){setComponents([]);setSelectedId(null);setValues({});}}} placeholder="Ex. Minoterie-semoulerie de blé, 100 t/j, puissance 180 kW, 3 silos total 14 000 m³..." />{query && <button type="button" onClick={clearActivity} className="absolute top-3 right-3 p-1.5 rounded-md text-gray-400 hover:text-red-600 hover:bg-red-50" title="Effacer l'activité"><X size={16}/></button>}</div></Field>
      ...
      {suggestions.length > 0 && <div className="mt-4"><div className="text-xs font-semibold text-gray-500 mb-2">Rubriques / activités suggérées</div><div className="border rounded-lg divide-y max-h-96 overflow-y-auto">{suggestions.map(c => <button key={`${c.rubrique}-${c.designation}`} className="w-full text-left px-3 py-3 hover:bg-emerald-50" onClick={() => addSuggestion(c)}><div className="text-sm font-medium">{c.designation}</div><div className="text-xs text-gray-500 mt-1">{c.familleLabel} · Rubrique {c.rubrique}</div></button>)}</div></div>}
