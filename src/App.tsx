...
function App() {
  ...
  const [allPaiements, setAllPaiements] = useState<PaiementWithDossier[]>([]);
  const [expertises, setExpertises] = useState<ExpertiseWithDossier[]>([]);
  const [allDocuments, setAllDocuments] = useState<DocumentWithDossier[]>([]);
  const [essais, setEssais] = useState<Laboratoire[]>([]);
  const [parametres, setParametres] = useState<Parametres | null>(null);

  const [openDossierId, setOpenDossierId] = useState<string | null>(null);
  const [dossierPaiements, setDossierPaiements] = useState<Paiement[]>([]);
  const [dossierExpertise, setDossierExpertise] = useState<Expertise | null>(null);
  const [dossierDocuments, setDossierDocuments] = useState<DocumentRow[]>([]);
  ...
  const loadAll = useCallback(async () => {
    ...
    setError('');
    try {
      const [c, d, p, e, docs, l, params] = await Promise.all([
        data.getClients(),
        data.getDossiers(),
  ...
  const openDossier = useCallback(async (id: string) => {
    ...
    setPage('dossiers');
    try {
      const [p, e, docs] = await Promise.all([
        data.getPaiementsByDossier(id),
        data.getExpertiseByDossier(id),
        data.getDocumentsByDossier(id),
      ]);
      setDossierPaiements(p);
  ...
  const refreshDossierDetail = useCallback(async () => {
    if (!openDossierId) return;
    try {
      const [p, e, docs] = await Promise.all([
        data.getPaiementsByDossier(openDossierId),
        data.getExpertiseByDossier(openDossierId),
        data.getDocumentsByDossier(openDossierId),
      ]);
      setDossierPaiements(p);
      setDossierExpertise(e);
      setDossierDocuments(docs);
  ...
  const handleUpdateDossier = async (id: string, d: Partial<Dossier>) => {
    await data.updateDossier(id, d);
    await loadAll();
    if (openDossierId === id) await refreshDossierDetail();
  };

  const handleDeleteDossier = async (id: string) => {
    await data.deleteDossier(id);
    setOpenDossierId(null);
    await loadAll();
  };

  const handleCreateClient = async (c: Omit<Client, 'id' | 'created_at'>) => {
    await data.createClient(c);
    await loadAll();
  };

  const handleUpdateClient = async (id: string, c: Partial<Client>) => {
    await data.updateClient(id, c);
    await loadAll();
  };
  ...
  const handleCreatePaiement = async (p: Omit<Paiement, 'id' | 'created_at'>) => {
    await data.createPaiement(p);
    await loadAll();
    await refreshDossierDetail();
  };

  const handleDeletePaiement = async (id: string) => {
    await data.deletePaiement(id);
    await loadAll();
    await refreshDossierDetail();
  };

  const handleCreateDocument = async (d: Omit<DocumentRow, 'id' | 'created_at'>) => {
    await data.createDocument(d);
    await loadAll();
    await refreshDossierDetail();
  };
  ...
  const handleUpdateDocumentPath = async (id: string, localPath: string) => {
    await data.updateDocumentPath(id, localPath);
    await loadAll();
    await refreshDossierDetail();
  };

  const handleVerifyDocument = async (id: string, statut: string) => {
    await data.updateDocumentStatut(id, statut);
    await loadAll();
    await refreshDossierDetail();
  };

  const handleCreateExpertise = async (e: Omit<Expertise, 'id' | 'created_at' | 'numero'>) => {
    if (openDossierId) {
      await data.createExpertise({ ...e, dossier_id: openDossierId });
    } else {
      await data.createExpertise(e);
    }
    await loadAll();
    await refreshDossierDetail();
  };

  const handleUpdateExpertise = async (id: string, e: Partial<Expertise>) => {
    await data.updateExpertise(id, e);
    await loadAll();
    await refreshDossierDetail();
  };

  const handleDeleteExpertise = async (id: string) => {
    await data.deleteExpertise(id);
    await loadAll();
    await refreshDossierDetail();
  };

  const handleCreateEssai = async (l: Omit<Laboratoire, 'id' | 'created_at' | 'numero_essai'>) => {
    await data.createLaboratoire(l);
    await loadAll();
  };

  const handleUpdateEssai = async (id: string, l: Partial<Laboratoire>) => {
    await data.updateLaboratoire(id, l);
    await loadAll();
  };

  const handleDeleteEssai = async (id: string) => {
    await data.deleteLaboratoire(id);
    await loadAll();
  };

  const handleSaveParametres = async (id: string, p: Partial<Parametres>) => {
    await data.updateParametres(id, p);
    await loadAll();
  };
  ...
  function renderPage() {
    ...
    if (page === 'dossiers' && openDossierId && openDossierData) {
      return (
        <FicheDossier
          dossier={openDossierData}
          clients={clients}
          paiements={dossierPaiements}
          expertise={dossierExpertise}
          documents={dossierDocuments}
          onBack={() => setOpenDossierId(null)}
          onEdit={(d) => {
            setEditDossier(d);
          ...
          onUpdateDocumentPath={handleUpdateDocumentPath}
          onVerifyDocument={handleVerifyDocument}
          onCreateExpertise={handleCreateExpertise}
  ...
  return (
    <div className="flex h-screen bg-gray-50 overflow-hidden">
      ...
      <DossierForm
        ...
        onSave={async (d) => {
          if (editDossier) await handleUpdateDossier(editDossier.id, d);
