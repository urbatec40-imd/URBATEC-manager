...
export function Sidebar({ current, onNavigate, open, onClose, session, onLogout }: SidebarProps) {
  return (
    <>
      ...
      <aside className={`fixed lg:static inset-y-0 left-0 z-40 w-64 bg-slate-800 text-slate-100 flex flex-col transition-transform duration-300 ${open ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'}`}>
        <div className="px-5 py-5 border-b border-slate-700">
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 rounded-lg bg-gradient-to-br from-sky-400 to-blue-600 flex items-center justify-center shadow-lg"><Building2 size={22} className="text-white" /></div>
        ...
        <nav className="flex-1 overflow-y-auto py-3">
          {NAV.map(({ key, label, icon: Icon }, index) => {
            ...
            return <button key={`${key}-${index}`} onClick={() => { onNavigate(key); onClose(); }} className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-colors ${active ? 'bg-sky-600 text-white border-l-4 border-sky-300' : 'text-slate-300 hover:bg-slate-700 hover:text-white border-l-4 border-transparent'}`}><Icon size={18} /><span>{label}</span></button>;
          })}
        </nav>
